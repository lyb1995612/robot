 ...
 * @project rest_demo
 */
package com.csjbot.robot.biz.sms.restDemo.client;


import com.csjbot.robot.biz.sms.restDemo.DateUtil;
import com.csjbot.robot.biz.sms.restDemo.EncryptUtil;
import com.csjbot.robot.biz.sms.restDemo.models.*;
...
public class JsonReqClient extends AbsRestClient {
	...
	public String findAccoutInfo(String accountSid, String authToken)
		...
		try {
			...
			logger.info(url);
			HttpResponse response=(HttpResponse) get("application/json",accountSid,authToken,timestamp,url,httpclient,encryptUtil);
			//获取响应实体信息
	...
	public String createClient(String accountSid, String authToken, 
		...
		try {
			...
			body="{\"client\":"+body+"}";
			HttpResponse response=(HttpResponse) post("application/json",accountSid, authToken, timestamp, url, httpclient, encryptUtil, body);
			//获取响应实体信息
			HttpEntity entity = response.getEntity();
			if (entity != null) {
				result = EntityUtils.toString(entity, "UTF-8");
