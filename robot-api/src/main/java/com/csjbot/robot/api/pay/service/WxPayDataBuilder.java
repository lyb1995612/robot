...
public class WxPayDataBuilder implements WxPayDataService {
    ...
    @Autowired
    public WxPayDataBuilder(WxPayPorperties prop,
                            @Qualifier("wxPayDBService") WxPayDBService dbService) {
        System.out.println("init WxPayDataBuilder");
        this.callbackUrl = prop.getValueStrict(WxPayPorperties.K_NOTIFY_URL);
        this.hostIp = prop.getServerIP();
        this.expireMin = prop.getExpireMinutes();
